...
consumers/security_eval/
  consumer.py            _handle_event() — all dispatch logic
                         _ONLINE_SIGNAL_MAP: maps 10 SDK signal names → Finding OWASP fields
...
db/postgres/             22 migrations (001–022)
  001_security_findings.sql     Main findings table (v3 confidence_tier column)
...
Makefile               run, setup, migrate, seed-*, health, test
...
dapplepot-api → POST /v1/evaluate
  ...
  │   └─ _ONLINE_SIGNAL_MAP[signal] → Finding fields
...
|--------|--------|----------------|----------------|
| OW-LLM01 Prompt Injection | ✅ | ✅ | PI-01a/b, PI-02a, PI-04b, PI-05a/06a/07a/08a/09a |
...
framework TEXT | severity TEXT | detection_phase TEXT
...
-- Dedup: ON CONFLICT (session_id, sub_check_id) take max check_score
```
...
| Add a new online signal mapping | `consumers/security_eval/consumer.py` → `_ONLINE_SIGNAL_MAP` |
